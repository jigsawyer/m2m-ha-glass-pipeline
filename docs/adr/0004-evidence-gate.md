...
A single one-column screenshot is not sufficient to pick a fix branch — classify first with `build/reports/floor_spacing_measure.js` and/or a remote↔local template diff. No evidence → HALT and request a measurement from the operator.

## Consequences

- Agents produce probe scripts (`build/reports/*_probe.js`) for the operator to run rather than iterating blind.
