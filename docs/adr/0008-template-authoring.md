...
**Thin instances.** Component templates (e.g. `button_primitive.jinja2`) reference only a base template name and entity state; all visual parameters live in the global `button_card_templates` dictionary.
...
Repeated glass/liquid patterns become shared Jinja2 macros.

## Consequences

- The template count grows only with new structure; an override request may correctly produce no `design_system/` write at all.
- A visual change is made once in `button_card_templates.yaml` or a token and inherited everywhere.
